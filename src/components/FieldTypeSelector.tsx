...
const fieldTypeOptions: FieldTypeOption[] = [
  {
    value: 'text',
    label: 'Text',
    description: 'Small to medium text strings',
    icon: Type,
  ...
  },
  {
    value: 'textarea',
    label: 'Long Text',
    description: 'Large blocks of text',
    icon: AlignLeft,
  ...
  },
  {
    value: 'checkbox',
    label: 'Checkbox',
    description: 'Multi-select boolean values',
    icon: CheckSquare,
...

  {
    value: 'date',
    label: 'Date',
    description: 'Calendars and anniversaries',
    icon: Calendar,
...

  {
    value: 'datetime',
    label: 'DateTime',
    description: 'Specific time and date',
    icon: Clock,
...
export function FieldTypeSelector({
  ...
  const selectedType =
    fieldTypeOptions.find((t) => t.value === value) || fieldTypeOptions[0]
  const SelectedIcon = selectedType.icon

  const filteredOptions = fieldTypeOptions.filter(
    (t) =>
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className='max-w-3xl max-h-[85vh] overflow-hidden flex flex-col p-0 gap-0 border-none rounded-3xl shadow-2xl'>
        ...
        <div className='flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar'>
          {categories.map((cat) => {
            const catOptions = filteredOptions.filter(
              (t) => t.category === cat.id
            )
                  ...
                  {catOptions.map((option) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={cn(
                            'p-2.5 rounded-xl mr-4 transition-transform group-hover:scale-110',
                            option.color
                          )}
                        >
