...
const RichTextEditor = ({ value, onChange, placeholder }: RichTextEditorProps) => {
  ...
  const editor = useEditor({
    extensions: [
      ...
      Image.extend({
        ...
        allowBase64: true,
        HTMLAttributes: {
          class: 'rounded-lg border shadow-sm max-w-full h-auto my-4 mx-auto block transition-all',
        },
      }),
    ],
    content: value,
  ...
  const handleUpdateImage = () => {
    editor.chain().focus().updateAttributes('image', {
      ...
      width: imageSettings.width ? (imageSettings.width.includes('%') || imageSettings.width.includes('px') ? imageSettings.width : `${imageSettings.width}px`) : null,
    }).run();
    ...
    <div className="rounded-md border bg-card overflow-hidden focus-within:ring-1 focus-within:ring-ring">
      <div className="flex flex-wrap items-center gap-1 border-b bg-muted/50 p-1 shrink-0">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-56">
            ...
            <DropdownMenuItem onClick={() => editor.chain().focus().deleteColumn().run()} disabled={!editor.isActive('table')} className="text-destructive focus:text-destructive">
            ...
            <DropdownMenuItem onClick={() => editor.chain().focus().deleteRow().run()} disabled={!editor.isActive('table')} className="text-destructive focus:text-destructive">
              <Trash2 className="mr-2 h-4 w-4" />
              <span>Delete Row</span>
            </DropdownMenuItem>
            <Separator className="my-1" />
            <DropdownMenuItem onClick={() => editor.chain().focus().deleteTable().run()} disabled={!editor.isActive('table')} className="text-destructive focus:text-destructive">
              <Trash2 className="mr-2 h-4 w-4" />
      ...
      <BubbleMenu editor={editor} shouldShow={({ editor }) => editor.isActive('image')}>
        <div className="flex border bg-card shadow-md rounded-md overflow-hidden p-1 gap-1">
          <Button type="button" variant="ghost" size="sm" onClick={editImageSettings} className="h-8 px-2 flex items-center gap-2">
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              />
              <p className="text-[10px] text-muted-foreground">Specify percentage (%) or pixels (px). Defaults to px if omitted.</p>
            </div>
          </div>
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setIsImageSettingsOpen(false)}>Cancel</Button>
            <Button type="button" onClick={handleUpdateImage}>Apply Changes</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
