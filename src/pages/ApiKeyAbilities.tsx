...
export default function ApiKeyAbilities({ user, abilities = [] }: { user?: any; abilities: Ability[] }) {
  const handleDelete = async (id: number, name: string) => {
    if (!confirm(`Are you sure you want to delete the ability "${name}"?`)) return;
      ...
      <div className="max-w-6xl mx-auto space-y-6 flex flex-col">
            ...
            <tbody className="divide-y">
              {abilities.length > 0 ? (
                abilities.map((ability) => (
                  <tr key={ability.id} className="hover:bg-muted/30 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {ability.isSystem === '1' && ability.name === 'Read Access' ? (
                           <span className="text-[10px] text-muted-foreground italic">Global Read Access</span>
                        ) : (
                          Object.entries(ability.permissions).map(([slug, perms]) => {
                            const active = Object.entries(perms).filter(([_, v]) => v).map(([k]) => k[0].toUpperCase());
                            if (active.length === 0) return null;
                            return (
                              <span key={slug} className="px-1.5 py-0.5 bg-muted rounded text-[10px] border">
                                {slug}: {active.join('')}
                              </span>
                            );
                          })
                        )}
                        {ability.isSystem !== '1' && Object.keys(ability.permissions).length === 0 && (
        ...
        <div className="p-4 bg-muted/40 rounded-lg border border-dashed flex items-start space-x-3">
          ...
          <div className="text-xs text-muted-foreground space-y-1">
             <p className="font-semibold text-foreground">Understanding Abilities</p>
             <p>Abilities define granular CRUD (Create, Read, Update, Delete) permissions for Collections. Once created, an ability can be assigned to any user from the User Management page.</p>
             <p className="italic">Note: Super Admins always have full access regardless of their assigned ability.</p>
          </div>
        </div>
      </div>
    </Layout>
