...
export default function EditForm({ form, user }: EditProps) {
  const { data, setData, put, processing, errors } = useForm({
    ...
    storageType: (form.storageType || 'external') as 'internal' | 'external',
    apiUrl: form.apiUrl || '',
    apiMethod: form.apiMethod,
    apiHeaders: form.apiHeaders as Record<string, string>,
    apiEntriesPath: form.apiEntriesPath || '',
    ...
    <Layout user={user}>
      ...
      <div className="w-full space-y-8">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-primary">Edit Form</h1>
            <p className="text-muted-foreground mt-1 underline decoration-primary/20">{form.name} definition</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="space-y-8">
            {/* General Settings */}
            <div className="bg-card p-6 rounded-xl border shadow-sm space-y-4 border-l-4 border-l-primary/30">
              ...
              <div className="space-y-2 opacity-60">
                <Label htmlFor="formSlug text-xs">Slug (Read Only)</Label>
                <Input 
                  id="formSlug" value={data.slug} disabled
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <div className="space-y-2">
                    {Object.entries(data.apiHeaders).map(([k, v]) => (
                      <div key={k} className="flex gap-2 items-center bg-background p-2 rounded-md group border border-muted/50">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Input placeholder="Key" value={headerKey} onChange={e => setHeaderKey(e.target.value)} className="h-8 text-xs bg-muted/10" />
                      <Input placeholder="Value" value={headerValue} onChange={e => setHeaderValue(e.target.value)} className="h-8 text-xs bg-muted/10" />
                    </div>
            ...
            <div className="flex flex-col gap-2">
              <Button type="submit" disabled={isSubmitting || processing} className="w-full">
                {isSubmitting ? 'Updating...' : 'Update Form Definition'}
              </Button>
