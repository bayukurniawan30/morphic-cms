...
export default function EditCollection({ collection, user }: EditProps) {
  const { data, setData, processing, errors } = useForm({
    ...
    localized: collection.localized || false,
              ...
              <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                ...
                <DialogContent className='max-w-3xl max-h-[90vh] flex flex-col'>
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Interactive preview of the JSON response for this global
                      singleton.
                    </DialogDescription>
                  </DialogHeader>
                  ...
                  <div className='flex-1 space-y-4 overflow-hidden flex flex-col mt-4'>
                    ...
                    <div className='flex-1 flex flex-col min-h-0 space-y-2'>
                      ...
                      <ScrollArea
                        className={`${collection.type === 'global' ? 'h-[450px]' : 'h-[350px]'} rounded-md border bg-zinc-950 p-4 font-mono text-xs text-zinc-300`}
                      >
          ...
          <div className='bg-card p-6 rounded-xl border shadow-sm space-y-4'>
            <div className='space-y-2'>
              <Label htmlFor='collectionName'>
                Collection Name <span className='text-destructive ml-1'>*</span>
              </Label>
              <Input
                id='collectionName'
              ...
              <p className='text-xs text-muted-foreground italic'>
                Slug: /{collection.slug} (Slug cannot be changed after creation)
              </p>
            </div>
                    ...
                    <div className='mt-4 p-4 bg-muted/30 rounded-md space-y-4 border border-muted-foreground/10 animate-in fade-in slide-in-from-top-2 duration-200'>
                      ...
                      {field.type === 'media' && (
                        <div className='flex items-center justify-between p-2 bg-background rounded-md border border-muted-foreground/10'>
                          ...
                          <Switch
                            ...
                            onCheckedChange={(val) =>
                              updateField(index, { multiple: val })
                            }
                          />
                        </div>
                      )}
                      ...
                      {field.type === 'relation' && (
                        <div className='space-y-4'>
                          <div className='space-y-2'>
                            ...
                            <Select
                              value={
                                field.relationCollectionId?.toString() || ''
                              }
                              onValueChange={(val) =>
                                updateField(index, {
                                  relationCollectionId: parseInt(val),
                          ...
                          {field.relationCollectionId && (
                            <div className='space-y-2'>
                              ...
                              >
                                ...
                                <SelectContent>
                                  {availableCollections
                                    .find(
                                      (c) => c.id === field.relationCollectionId
                                    )
                                    ?.fields.map((f: any) => (
                              ...
                              <p className='text-[10px] text-muted-foreground italic'>
                                This field will be shown in the dropdown when
                                creating entries.
                              </p>
                            </div>
                          )}
                      ...
                      {field.type === 'slug' && (
                        <div className='space-y-2 text-left'>
                          <Label className='text-xs'>
                            Base Field (for auto-generation)
                          </Label>
                          <Select
                            value={field.slugSourceField || ''}
                            onValueChange={(val) =>
                              updateField(index, { slugSourceField: val })
                            }
                          >
                            <SelectTrigger className='h-8 text-xs bg-background'>
                      ...
                      {['date', 'datetime', 'time', 'rich-text'].includes(
                        field.type
                      ) && (
                        <p className='text-[10px] text-muted-foreground italic'>
