      ...
      <div className='w-full space-y-6 flex flex-col'>
        <div className='flex justify-between items-end'>
          <div>
            ...
            <p className='text-muted-foreground text-sm'>
              Configure and test transactional emails via Resend.
            </p>
          </div>
        </div>
        ...
        <div className='bg-card rounded-xl border p-6 space-y-6 shadow-sm'>
          ...
          <form onSubmit={handleSendTest} className='space-y-4'>
            ...
            {result && (
              <div
                className={`p-4 rounded-lg flex items-start space-x-3 text-sm transition-all animate-in fade-in slide-in-from-top-1 ${
                  result.success
        ...
        <div className='p-4 bg-muted/40 rounded-lg border border-dashed flex items-start space-x-3'>
          <div className='text-xs text-muted-foreground space-y-1'>
            ...
            <p>
              Transactional emails can be sent from any server-side component
              using the <code>src/lib/email.ts</code> utility. Remember to
              verify your domain or the recipient's address in Resend's
              dashboard if you're using a free/sandbox account.
            </p>
          </div>
        </div>
      </div>
