...
import { SettingsIcon, MailIcon, ShieldCheckIcon, DatabaseIcon } from 'lucide-react';
    ...
    <Layout user={user}>
      ...
      <div className="max-w-4xl mx-auto space-y-8 flex flex-col pt-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">Centralized configuration for your Morphic CMS instance.</p>
        </div>

        <div className="space-y-8 text-sm">
          {settingGroups.map((group, idx) => (
            <div key={idx} className="space-y-4">
              <h2 className="text-xs font-bold uppercase tracking-widest text-muted-foreground ml-1">{group.title}</h2>
                ...
                {group.items.map((item, itemIdx) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-4">
                      ...
                      {item.disabled && <span className="text-[10px] font-bold uppercase tracking-widest bg-muted px-2 py-0.5 rounded text-muted-foreground">Coming Soon</span>}
                    </div>
                    <div>
